...
const DRAFT = {
  project: 'Storefront',
  milestones: [{ id: 'm1', title: 'Buying flow' }, { id: 'm2', title: 'Warehouse' }],
  items: [
    ...
    { id: 'warehouse-display', title: 'Warehouse display', milestone: 'm2', status: 'todo' },
  ],
};
...
test('slug turns titles into ids', () => {
  ...
  assert.strictEqual(bootstrap.slug('  Checkout with reserved copies  '), 'checkout-with-reserved-copies');
...
test('README headings skip fenced blocks and go three levels deep', () => {
  ...
  fs.writeFileSync(path.join(dir, 'README.md'), [
    '# Storefront', '', 'Intro', '', '## Buying flow', '', '```sh', '# not a heading', '```', '', '### Warehouse display ###', '#### Too deep', '',
  ].join('\n'));
  assert.deepStrictEqual(bootstrap.readmeHeadings(dir), [
    { level: 1, text: 'Storefront' },
    { level: 2, text: 'Buying flow' },
    { level: 3, text: 'Warehouse display' },
  ]);
...
test('the prompt lists every source and names the language', () => {
  const prompt = bootstrap.buildPrompt({
    ...
    sources: {
      project: 'storefront',
      commits: [{ date: '2026-09-01', subject: 'Add listing editor' }],
      headings: [{ level: 2, text: 'Buying flow' }],
      prs: [
        { number: 42, title: 'Listing editor', state: 'closed', merged: true, draft: false },
        { number: 44, title: 'Checkout', state: 'open', merged: false, draft: true },
      ],
    },
  });
  assert.ok(prompt.includes('Directory name: storefront'));
  assert.ok(prompt.includes('- 2026-09-01 Add listing editor'));
  assert.ok(prompt.includes('- ## Buying flow'));
  assert.ok(prompt.includes('- #42 [merged] Listing editor'));
  assert.ok(prompt.includes('- #44 [open, draft] Checkout'));
  assert.ok(prompt.includes('in German'));
  assert.ok(prompt.includes('"id": "listing-editor"'), 'schema example');
...
test('init drafts the first roadmap from git history and the README', async () => {
  const dir = repo(['Add listing editor', 'Start checkout'], '# Storefront\n\n## Buying flow\n');
  ...
  assert.ok(prompts[0].prompt.includes('- ## Buying flow'));
...
test('init reads pull request titles without the per-PR review and comment calls', async () => {
  ...
  const prompts = [];
  const calls = [];
  await runInit({ file: path.join(dir, 'roadmap.json'), repo: 'acme/storefront' }, baseEnv, {
    log: () => {}, exec: fakeExec(), credential: { token: 't' }, fetchFn: fakeGithubFetch({ calls }), provider: draftProvider({ prompts }),
  });
  assert.ok(calls.length > 0);
  assert.ok(calls.every((u) => new URL(u).pathname === '/repos/acme/storefront/pulls'), calls.join('\n'));
...
test('--no-bootstrap writes exactly the old skeleton and asks nobody', async () => {
  const dir = repo(['Add listing editor']);
  const file = path.join(dir, 'roadmap.json');
  const calls = { n: 0 };
