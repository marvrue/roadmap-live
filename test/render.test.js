...
test('focus: active items at full size, blocked after them, now block folded in', () => {
  ...
  assert.ok(/<h2 class="title">Checkout with reserved copies<\/h2><div class="elapsed" data-since="2026-09-09T09:00:00Z">/.test(html));
  assert.ok(/<h2 class="title">Card payment with Stripe<\/h2><span class="label attention state">blocked<\/span>/.test(html));
  assert.ok(html.includes('href="https://github.com/acme/storefront/pull/44#discussion_r2001"'), 'open points link to the review comment');
  assert.ok(html.includes('Hold copies for 15 or 30 minutes?') && html.includes('data-action="answer" data-id="checkout" data-text="15"'));
...
test('conversations: questions first, unanswered threads marked, waiting block folded in, others foldable with the key', () => {
  ...
  assert.ok(main.includes('Hold copies for 15 or 30 minutes?'));
...
test('open points: grouped by item, open first, resolved folded, linked to the comment', () => {
  ...
  assert.deepStrictEqual(ids, ['<section class="group" data-id="checkout"', '<section class="group is-done" data-id="listing-editor"']);
  assert.ok(main.includes('<span class="meta">2 open</span>'));
  assert.ok(main.includes('<details class="fold" data-key="pt:listing-editor"><summary>1 resolved</summary>'));
  assert.ok(main.includes('href="https://github.com/acme/storefront/pull/42#issuecomment-1893"'));
...
test('pull requests: newest first with items, review points and unplanned work', () => {
  ...
  assert.ok(/data-pr="41">[\s\S]*?<span class="label attention">Unplanned<\/span><span class="text">Wishlist alerts<span class="tag">Not on the roadmap<\/span><\/span>/.test(main));
  assert.ok(/data-pr="44">[\s\S]*?<span class="label">review<\/span><span class="text">Checkout with reserved copies: Please debounce/.test(main));
...
test('list: one table row per item, default order by milestone then status, headers sort', () => {
  ...
  assert.deepStrictEqual(ids.slice(0, 3), ['payment', 'checkout', 'listing-editor']);
  assert.ok(main.includes('<td class="c-status mono attention">blocked</td>'));
  assert.ok(main.includes('<tr data-id="listing-editor" class="is-done">'));
...
test('now panel: an empty current milestone points at the first open item anywhere, none open means all done', () => {
  ...
  const data = fixture('roadmap-base.json');
  data.milestones.unshift({ id: 'm0', title: 'Empty first' });
  data.items.find((it) => it.id === 'listing-editor').status = 'done';
  ...
  assert.ok(/<div class="idle">Next up: <b>Checkout with reserved copies<\/b>/.test(html));
...
test('tagline: whitespace only is no tagline, no data falls back to the tool name', () => {
  ...
  assert.ok(html.includes('<div class="brand"><h1>storefront</h1></div>'));
...
test('German: the unmeasured goal title is translated', () => {
  ...
  assert.ok(/<span class="label status goal" title="noch nicht gemessen">–\/5<\/span><span>Packing slip printer support<\/span>/.test(html));
});
...
test('focus: a filter without active items shows the idle block; all done and no items have their own sentence', () => {
  ...
  assert.ok(filtered.includes('Everything is done.') && filtered.includes('3 milestones, 11 items.'), 'the idle block ignores the filter: Buying flow is current and has no open item left');
  ...
  assert.ok(idle.includes('Nothing in progress right now.') && idle.includes('Next up: <b>Checkout with reserved copies</b>'));
...
test('conversations: empty sentence on the static page, filter narrows the threads, blocked marked, fold remembers being open', () => {
  ...
  assert.deepStrictEqual(ids, ['<article class="item" data-id="checkout"', '<article class="item" data-id="payment"'], 'Warehouse threads are filtered out');
  assert.ok(/data-id="payment"[\s\S]*?<span class="attention">blocked<\/span>/.test(m1));
  assert.ok(!m1.includes('data-key="conv:others"'), 'every open item of Buying flow has a thread');
...
test('open points: only resolved points show the empty sentence, a filter narrows the groups, fold remembers being open', () => {
  ...
  const main = mainOf(view.renderApp(resolved, { ...BASE_OPTS(), view: 'points', open: { 'pt:listing-editor': true } }), 'points');
  assert.ok(main.startsWith('<main class="points"><div class="empty">No open points. Reviewers have nothing outstanding.</div>'));
  assert.deepStrictEqual(main.match(/<section class="group[^"]*" data-id="([a-z-]+)"/g), ['<section class="group is-done" data-id="checkout"', '<section class="group is-done" data-id="listing-editor"'], 'last resolved first');
  assert.ok(main.includes('<details class="fold" data-key="pt:listing-editor" open><summary>1 resolved</summary>'));
...
test('pull requests: a filter drops unplanned work, the empty sentence where nothing is linked, fold remembers being open', () => {
  ...
  assert.ok(!m1.includes('Wishlist alerts'));
...
test('signals: a filter hides unplanned work and the calm counts follow it; rows without a pull request keep the plain label', () => {
  ...
  assert.ok(!m2.includes('Wishlist alerts'));
  ...
  assert.ok(/<span class="label attention">Unplanned<\/span><span class="text">Wishlist alerts<\/span>/.test(html));
});
...
test('list: every column sorts both ways, an unknown key keeps the default order', () => {
  ...
  assert.deepStrictEqual(first({ key: 'status', dir: 'desc' }), { id: 'listing-editor', head: '<th class="c-status" aria-sort="descending">' });
...
test('questions: no options renders no buttons, a done item drops its question, the live page without the key is read-only', () => {
  ...
  assert.ok(noOptions.includes('Hold copies for 15 or 30 minutes?<div class="answers"></div>'));
  ...
  assert.ok(focus.includes('Hold copies for 15 or 30 minutes?</div>'), 'the question in the thread brings no hint of its own');
  ...
  assert.ok(!gone.includes('Hold copies for 15 or 30 minutes?'));
});
...
test('an unknown milestone shows its id; points link to reviews and plain pull requests, and lose the link without a repository', () => {
  ...
  assert.ok(focus.includes('href="https://github.com/acme/storefront/pull/44#pullrequestreview-7"'));
  assert.ok(focus.includes('<a href="https://github.com/acme/storefront/pull/44">PR #44</a></span><span class="text">Add a test for the empty cart case</span>'));
...
test('an item with several pull requests keeps the "PRs #44, #45" subline (locale key must not be shadowed)', () => {
  ...
  assert.ok(/PRs <a href="https:\/\/github.com\/acme\/storefront\/pull\/44">#44<\/a>, <a [^>]*>#45<\/a>/.test(html));
...
test('focus idle text follows the milestone filter', () => {
  ...
  assert.ok(unfiltered.includes('<b>Checkout with reserved copies</b>'), 'without a filter the current milestone leads');
});
