...
function run(file, { dryRun = false, now = NOW, log = [], provider = fakeProvider() } = {}) {
  ...
  return runSync({ file, repo: 'acme/abholbereit', dryRun, apply: false, provider: null }, env, {
...
test('sync end to end: statuses, open points, unplanned, changelog, key order', async () => {
  ...
  assert.deepStrictEqual(after.unplanned, [{ title: 'Customer loyalty points', prs: [41], first_seen: '2026-09-06T09:00:00Z' }]);
  assert.deepStrictEqual(after.sync, { last_run: '2026-09-10T12:00:00.000Z', repo: 'acme/abholbereit' });
  ...
  assert.deepStrictEqual(Object.keys(by('menu-editor')), ['id', 'title', 'milestone', 'status', 'updated', 'prs', 'branch']);
  ...
  assert.ok(changelog.includes('Menu editor active → done (https://github.com/acme/abholbereit/pull/42)'));
  assert.ok(changelog.includes('Checkout with pickup time todo → active'));
...
test('--dry-run prints the plan and writes nothing', async () => {
  ...
  assert.ok(log.some((l) => l.includes('Menu editor: active → done (PR #42)')));
...
test('a PR that fails classification is skipped and fetched again next time', async () => {
  ...
  const after = JSON.parse(fs.readFileSync(file, 'utf8'));
  assert.strictEqual(after.items.find((it) => it.id === 'menu-editor').status, 'done', 'other PRs are still applied');
