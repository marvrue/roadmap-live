...
test('agent provider round trip: inbox written, result applied, files removed', async () => {
  ...
  const log = [];
  const opts = { file, repo: 'acme/storefront', dryRun: false, apply: false, provider: null };
  ...
  assert.ok(inbox.items.some((it) => it.id === 'listing-editor'));
  ...
  assert.deepStrictEqual(by('listing-editor').prs, [42]);
  ...
  assert.strictEqual(after.sync.repo, 'acme/storefront');
  assert.ok(!fs.existsSync(path.join(dir, INBOX_DIR, PENDING_FILE)));
  assert.ok(!fs.existsSync(path.join(dir, INBOX_DIR, RESULT_FILE)));
  assert.ok(fs.readFileSync(path.join(dir, 'CHANGELOG.md'), 'utf8').includes('Listing editor active → done'));
});
