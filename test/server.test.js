...
function pagesFetch(state) {
  return async (url, init = {}) => {
    ...
    if (init.method === 'POST') { state.enabled = true; return new Response(JSON.stringify({ html_url: 'https://acme.github.io/storefront/' }), { status: 201 }); }
    return state.enabled ? new Response(JSON.stringify({ html_url: 'https://acme.github.io/storefront/' }), { status: 200 }) : new Response('{"message":"Not Found"}', { status: 404 });
  };
}
