...
test('syncRoadmap returns the next roadmap and the changes without writing anything', async () => {
  ...
  assert.deepStrictEqual(r.roadmap.sync, { last_run: '2026-09-10T12:00:00.000Z', repo: 'acme/storefront' });
  assert.ok(r.changes.some((c) => c.kind === 'status' && c.title === 'Listing editor' && c.to === 'done'));
  const lines = lib.changelogLines(r.changes, r.t, 'en');
  assert.ok(lines.some((l) => l.includes('Listing editor active → done')));
...
test('syncRoadmap with no new activity reports prs: [] and leaves the roadmap alone', async () => {
  const roadmap = fixture('roadmap-base.json');
  roadmap.sync = { last_run: '2026-09-10T11:00:00Z', repo: 'acme/storefront' };
  const r = await lib.syncRoadmap({ roadmap, repo: 'acme/storefront', token: 't', provider: fakeProvider(), fetchFn: fakeGithubFetch(), now: NOW, env: baseEnv });
...
test('syncRoadmap keeps a failed pull request for the next run', async () => {
  ...
  const r = await lib.syncRoadmap({ roadmap, repo: 'acme/storefront', token: 't', provider, fetchFn: fakeGithubFetch(), now: NOW, env: baseEnv });
  assert.deepStrictEqual(r.failed.map((pr) => pr.number), [43]);
  assert.strictEqual(r.roadmap.sync.last_run, '2026-09-08T10:59:59.000Z');
  assert.strictEqual(r.roadmap.items.find((it) => it.id === 'listing-editor').status, 'done');
});
