...
test('real classification of the merged menu editor PR', { skip: !enabled && 'set ROADMAP_LIVE_TEST=1' }, async () => {
  const { provider } = resolveProvider({ env: process.env });
  assert.ok(provider && !provider.handoff, 'a key or a CLI is needed');
  const pr = {
    number: 42, title: 'Menu editor with image upload', url: 'https://github.com/acme/abholbereit/pull/42',
    body: 'Menu editor for the restaurant owner. Images are resized on upload.', author: 'mara',
  ...
  const match = r.value.matches.find((m) => m.item_id === 'menu-editor');
