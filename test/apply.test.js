...
test('merged PR above 0.75 sets done and links the PR', () => {
  ...
  const item = roadmap.items.find((it) => it.id === 'listing-editor');
...
test('merged PR with confidence 0.6 links the PR but leaves the status', () => {
  const roadmap = fixture('roadmap-base.json');
  applyClassification(roadmap, prs()[42], { matches: [{ item_id: 'listing-editor', confidence: 0.6, status_hint: 'done' }], open_points: [], unplanned: [] });
  const item = roadmap.items.find((it) => it.id === 'listing-editor');
...
test('an open follow-up PR does not reopen a done item', () => {
  ...
  applyClassification(roadmap, prs()[44], { matches: [{ item_id: 'listing-editor', confidence: 0.9, status_hint: 'active' }], open_points: [], unplanned: [] });
...
test('unrelated PR goes to unplanned once and leaves it when matched later', () => {
  ...
  applyClassification(roadmap, pr, fixture('classify/pr-41.json'));
  assert.deepStrictEqual(roadmap.unplanned, [{ title: 'Wishlist alerts', prs: [41], first_seen: '2026-09-06T09:00:00Z' }]);
