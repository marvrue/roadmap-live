...
// A fetch that serves the recorded GitHub fixtures for acme/storefront.
...
function fakeGithubFetch({ calls = [] } = {}) {
  ...
  return async (url) => {
    ...
    const u = new URL(url);
    const p = u.pathname;
    if (p === '/repos/acme/storefront/pulls') {
    ...
    let m = /^\/repos\/acme\/storefront\/pulls\/(\d+)\/reviews$/.exec(p);
    if (m) return respond(fixture(`github/pr-${m[1]}-reviews.json`));
    m = /^\/repos\/acme\/storefront\/issues\/(\d+)\/comments$/.exec(p);
    if (m) return respond(fixture(`github/pr-${m[1]}-comments.json`));
    m = /^\/repos\/acme\/storefront\/pulls\/(\d+)\/comments$/.exec(p);
