...
test('rejects malformed goals', () => {
  ...
  const errors = validate(data);
  assert.ok(errors.some((e) => e.includes('items[0] ("listing-editor").goal.label must be a non-empty string')));
...
test('rejects goals whose target or source has the wrong type', () => {
  ...
  const errors = validate(data);
  assert.ok(errors.some((e) => e.includes('items[0] ("listing-editor").goal.label must be a non-empty string')));
  assert.ok(errors.some((e) => e.includes('items[0] ("listing-editor").goal.target must be a finite number greater than 0')));
...
test('rejects infinite numbers: 1e400 parses as Infinity and would serialize as null', () => {
  ...
  const errors = validate(data);
  assert.ok(errors.some((e) => e.includes('items[0] ("listing-editor").goal.target must be a finite number greater than 0')));
  assert.ok(errors.some((e) => e.includes('items[0] ("listing-editor").goal.current must be a finite number of 0 or more')));
});
...
test('npm-downloads source: package names up to 214 characters, not longer', () => {
  ...
  assert.ok(validate(data).some((e) => e.includes('items[0] ("listing-editor").goal.source for npm-downloads must be an npm package name')));
});
...
test('tagline must be a non-empty string when present', () => {
  const data = fixture('roadmap-base.json');
  data.tagline = 'Secondhand records from a small shop';
  assert.deepStrictEqual(validate(data), []);
  data.tagline = '';
...
test('warnings: done item under target without a source', () => {
  ...
  assert.deepStrictEqual(warnings(data), [{ key: 'goalDoneUnderTarget', item: 'listing-editor', current: 40, target: 100 }]);
});
...
test('warnings: a done item with current 0 still counts as under target', () => {
  ...
  assert.deepStrictEqual(validate(data), []);
  assert.deepStrictEqual(warnings(data), [{ key: 'goalDoneUnderTarget', item: 'listing-editor', current: 0, target: 10 }]);
});
...
test('--check prints goal and tagline notes after the ok line', () => {
  ...
  assert.ok(logs.some((l) => l.includes('note: "listing-editor" is done but its goal stands at 40/100')));
...
test('--check prints goal and tagline notes in German', () => {
  ...
  assert.ok(logs.some((l) => l.includes('Hinweis: "listing-editor" ist erledigt, aber das Ziel steht bei 40/100')));
...
test('--check strips control characters from item ids in notes', () => {
  const data = fixture('roadmap-base.json');
  data.items[0].id = 'listing\x1b[2K\nfake\u202e';
  data.items[0].status = 'done';
  data.items[0].goal = { label: 'Stars', target: 100, current: 40 };
  const { code, logs } = checkOutput(data, { LANG: 'C' });
  assert.strictEqual(code, 0);
  assert.ok(logs.some((l) => l.includes('note: "listing[2Kfake" is done but')));
...
test('--check fails on a malformed goal and prints no notes', () => {
  ...
  assert.ok(errs.some((l) => l.includes('items[0] ("listing-editor").goal.target must be a finite number greater than 0')));
});
