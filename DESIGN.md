...
- **Label:** mono 12px in `text-3`; `attention` when the row is blocked, stale, unplanned or a question. Labels wrap (`white-space: normal`) so "Checkout with reserved copies" can be a label on a phone.
